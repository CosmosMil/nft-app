...
const Request = () => {
  ...
  const handleYesClick = async (swap: Swap) => {
        ...
        <div className='flex flex-wrap items-start justify-center'>
          {requests && requests.map((swap: Swap) => (
            <div key={swap._id} className='p-3'>
              ...
              <div className='text-center text-yellow-100 p-8'><button className='mx-4 w-12 hover:bg-indigo-400 rounded' onClick={async () => await handleYesClick(swap)}>YES</button> <i className="fa-solid fa-arrow-down"></i> <i className="fa-solid fa-arrow-up"></i><button className='mx-4 w-12 hover:bg-indigo-400 rounded'>NO</button></div>
              <div className='h-80 w-54 p-5 my-5 border-2 border-dotted  border-indigo-400'>
